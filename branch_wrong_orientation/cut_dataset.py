...
if __name__ == "__main__":
    ...
    merged_dir = "merged"
    pattern_z = re.compile(r"Z(\d+)_")

    for filename in sorted(os.listdir(merged_dir)):
        if filename.endswith(".png"):
            # Шлях до зображення
            image_path = os.path.join(merged_dir, filename)
            name_only = os.path.splitext(filename)[0]

            # Шукаємо номер Z
            z_match = pattern_z.search(filename)
            z_number = int(z_match.group(1)) 
            
            
            # Завантажуємо саме зображення
            image = cv2.imread(image_path)
            zone_number = z_number
            ...
            if workspace is not None:
                ...
                if new_rois:
                    for roi_name, roi_image in new_rois.items():
                        cv2.imwrite(f"branchdataset/{name_only}_{roi_name}.png",roi_image)
