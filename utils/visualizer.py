...
class Visualizer:
    ...
    def draw_rois(
        self, roi_cropper, color=None, label_prefix="", color_by_category=False
    ):
        """
        ...
        for idx, (category_name, roi_list) in enumerate(
            roi_cropper.roi_objects.items()
        ):
            # Choose color
    ...
    def draw_rois_by_category(self, roi_cropper, category_name, color, label_prefix=""):
        ...
        for roi in roi_list:
            ...
            type_indicator = "■" if roi_type == "center" else "▭"
            label = (
                f"{label_prefix}{category_name.upper()}_{roi_id:03d} {type_indicator}"
            )
