                    ...
                    for name in dict_names:
                        ...
                        if not dict_node.empty():
                            try:
                                ...
                                self.logger.error(f"Error loading custom dictionary {name}: {e}")
                                continue
                    fs.release()
                else:
                    self.logger.warning(f"Could not open YAML file: {self.custom_yaml_path}")
            except Exception as e:
                self.logger.error(f"Error reading custom dictionary file: {e}")
        else:
            self.logger.warning(f"Custom dictionary file not found: {self.custom_yaml_path}")
